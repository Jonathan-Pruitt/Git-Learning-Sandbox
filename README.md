...
Step 2. Choose a location for your project
    ...
    **Step 2 *(Verbose)***
    ...
    - In 'Bash', you can use `ls` to see the all of the files and folders that are present in your current working directory (cwd).
...
`git -v`
If it returns something like: `git version {number}.{number}.{number}` (e.g. `git version 2.43.0`), then it is already installed and you can skip to [Cloning a Repository](#cloning-a-repository). Otherwise, continue reading.
...
`sudo apt update` # the *sudo* (a.k.a. 'Super Do') prefix modifies the security context of the command (escalating your current level of permissions to the 'admin' level for the duration of this command)
